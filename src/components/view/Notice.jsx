...
function Notice() {
  ...
  const notices = [
    {
      id: 1,
      title: 'Notice Regarding Annual Meeting',
      date: 'June 20, 2025',
      fileUrl: image,
      ...
      {!selectedNotice ? (
        <>
          <div className="mb-8">
            ...
            <p className="text-base text-gray-600 mt-2">Click on a notice to view more details or download the file.</p>
          </div>
  ...
  >
    <div className="mb-4 sm:mb-0">
      <p className="text-lg sm:text-xl font-semibold text-gray-900">{notice.title}</p>
     ...
     className="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-red-600 flex items-center"
      ...
      >
        ...
        <div className="space-y-6 flex-grow overflow-auto">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">{selectedNotice.title}</h2>
          ...
          <div className="border border-gray-300 rounded-lg p-4 sm:p-6 bg-gray-50 flex justify-center">
            {selectedNotice.fileType === 'image' ? (
              ...
              <iframe src={selectedNotice.fileUrl} className="w-full h-[400px] sm:h-[500px]" title="Notice File" />
            )}
          </div>
          ...
          <p className="text-sm text-gray-500 mt-4">Published: {selectedNotice.date}</p>
        </div>
      )}
    </div>
  );
