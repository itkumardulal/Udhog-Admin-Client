...
const Company = () => {
  const headers = {
    ...
    leadershipGender: "Leadership Gender",
    renewStatus: "Status",
    actions: "Actions",
  };
  ...
  const filterConfig = [
    ...
    {
      label: "By Business Nature",
      name: "businessNature",
      type: "select",
      options: ["All", "Udhog", "Badihya", "Nikaya"],
    },
    {
      label: "By Leadership Gender",
      name: "leadershipGender",
      type: "select",
      options: ["All", "Male", "Female", "Others"],
    },
    {
      label: "By Membership Type",
      name: "membershipType",
      type: "select",
      options: [
        "All",
  ...
  // Filtering logic based on filters object keys and values
  const filteredCompanies = companies.filter((company) => {
    if (filters.status !== "All" && company.renewStatus !== filters.status)
      return false;

    if (
      filters.businessNature !== "All" &&
      company.business !== filters.businessNature
    )
      return false;

    if (
      filters.leadershipGender !== "All" &&
      company.leadershipGender !== filters.leadershipGender
    )
      return false;

    if (
      filters.membershipType !== "All" &&
      company.membershipType !== filters.membershipType
    )
      return false;
  ...
  return (
    <div className="min-h-screen p-8 bg-gray-100">
      {/* FILTERS SECTION */}
      <div className="bg-white shadow-lg rounded-xl p-8 mb-8 space-y-8 max-w-7xl mx-auto">
        <div className="flex justify-between items-center border-b border-gray-200 pb-4">
          ...
          <button
            ...
            className=" hover:cursor-pointer flex items-center gap-2 bg-blue-600 text-white px-5 py-2.5 rounded-lg shadow-md hover:bg-blue-700 transition duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {filterConfig.map(({ label, name, type, options, placeholder }) => (
            <div key={name} className="flex flex-col">
              ...
              {type === "select" ? (
                <select
                  ...
                  className="w-full md:w-64 border border-gray-300 rounded-lg px-4 py-3 text-sm text-gray-800 bg-white shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                >
                ...
                <input
                  ...
                  value={filters[name]}
                  onChange={(e) => handleFilterChange(name, e.target.value)}
                  className="w-full border border-gray-300 rounded-lg px-4 py-3 text-sm placeholder-gray-400 text-gray-800 shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-400"
                />
              )}
            </div>
          ))}
                ...
                {filteredCompanies.length > 0 ? (
                  filteredCompanies.map((company, index) => (
                    <tr key={company.id} className="hover:bg-gray-50">
                      ...
                      <td className="p-4">{company.membershipNo}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4">{company.telPhone}</td>
                      <td className="p-4">{company.phoneNo}</td>
                      <td className="p-4">{company.vat || "-"}</td>
                      <td className="p-4">{company.pan || "-"}</td>
                      <td className="p-4">{company.capital}</td>
                      <td className="p-4">{company.numberOfEmployees}</td>
                      <td className="p-4">
                        {company.registrationDate?.slice(0, 10)}
                      </td>
                      <td className="p-4">
                        {company.membershipDate?.slice(0, 10)}
                      </td>
                      <td className="p-4">{company.businessNature}</td>
                      <td className="p-4">{company.membershipType}</td>
                      <td className="p-4">{company.leadershipGender}</td>
                      <td className="p-4">
                      ...
                      <td className="p-4">
                        <div className="flex gap-2 items-center">
                          {/* view svg */}
                          <Link to={`/view/company/${company.id}`}>
                            <button className="p-2 rounded-full bg-white group hover:bg-gray-200 transition-all hover:cursor-pointer">
                              <svg
